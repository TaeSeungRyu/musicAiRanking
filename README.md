...
- **채널 URL 입력 시 전체 영상을 한 번에 일괄 추가** (InnerTube continuation 페이지네이션 — 첫 30개가 아닌 전체)
...
- **기본 대상 채널은 설정 화면에 보이지만 제거 불가**, **1시간 단위 자동 최신화** (서버 내장 스케줄러)
- 채널 재수집 시 **기존 데이터 제거 후 재등록** (삭제·비공개된 영상이 남지 않음)
- 조회수 내림차순 랭킹 자동 정렬
- 클릭 시 원본 YouTube로 이동
...
instrumentation.ts      대상 채널 1시간 단위 최신화 스케줄러 (서버 부팅 시 등록)
lib/
